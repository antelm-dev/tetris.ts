...
/** Upper bound accepted from the client for a score or a duration (ms) — finite, integral, non-negative. */
...
/**
 * File-backed per-mode records store for the web target, the browser-facing
 * twin of `main/ipc/game.ipc.ts`'s Electron version: same versioned schema,
 * same validation, same write-serialization so concurrent submits can't race
 * on the file. Takes an explicit path (rather than Electron's
 * `app.getPath('userData')`) so it's trivial to point at a temp file in tests.
 */
